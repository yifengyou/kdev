...
ISOURL="https://mirrors.openanolis.cn/anolis/8.8/isos/GA/aarch64/AnolisOS-8.8-aarch64-dvd.iso"
WORKDIR=$(pwd)
...
if [ ! -f mnt/images/pxeboot/vmlinuz-5.10.134-13.an8.aarch64 ]; then
	echo "mnt/images/pxeboot/vmlinuz-5.10.134-13.an8.aarch64 does't exists!"
	exit 1
fi

if [ ! -f mnt/images/pxeboot/initrd-5.10.134-13.an8.aarch64.img ]; then
	echo "mnt/images/pxeboot/initrd does't exists!"
	exit 1
fi
...
qemu-system-aarch64 \
	...
	-cdrom ${ISONAME} \
	-boot order=dc \
	-kernel mnt/images/pxeboot/vmlinuz-5.10.134-13.an8.aarch64 \
	-initrd mnt/images/pxeboot/initrd-5.10.134-13.an8.aarch64.img \
	-append "inst.ks=http://192.168.122.1:${FILE_SERVER_PORT}/ks.cfg inst.stage2=hd:LABEL=${LABEL} inst.text inst.cmdline earlyprintk ignore_loglevel console=ttyAMA0,115200n8 earlycon=pl011,mmio,0x09000000 level=10 net.ifnames=0 biosdevname=0 " \
	-serial mon:stdio \
	-net nic \
