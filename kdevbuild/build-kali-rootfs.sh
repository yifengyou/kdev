...
process_qcow2() {
  ...
  ls -alh ${WORKDIR}/kali/rootfs.img
  file ${WORKDIR}/kali/rootfs.img
  ...
  md5sum ${WORKDIR}/release/${QCOW2%.tar.xz}.rar

  ls -alh ${WORKDIR}/release/
  df -h
}

# https://kali.download/cloud-images/kali-2025.4/kali-linux-2025.4-cloud-genericcloud-amd64.tar.xz

rm -f index.html disk.raw rootfs.img *.qcow2
if wget --no-check-certificate "https://kali.download/cloud-images/${set_version}/"; then
  ls -alh index.html
  for QCOW2 in $(grep -oE 'href="([^"]+\.tar.xz)"' index.html | cut -d'"' -f2); do
    rm -f disk.raw rootfs.img
    export QCOW2_URL="https://kali.download/cloud-images/${set_version}/${QCOW2}"
