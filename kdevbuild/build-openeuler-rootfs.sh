...
if ls -alh ${WORKDIR}/release/${QCOW2%.qcow2}.rar; then
  echo "Ok! skip archive! Build completed successfully!"
  exit 0
fi

rm -f index.html qcow2.raw rootfs.img *.qcow2 *.xz
if wget --no-check-certificate "https://archives.openeuler.openatom.cn/openeuler/${set_version}/virtual_machine_img/aarch64/"; then
  ls -alh index.html
  for QCOW2 in $(grep -oE 'href="([^"]+\.qcow2.xz)"' index.html | cut -d'"' -f2); do
    rm -f qcow2.raw rootfs.img
    export QCOW2_URL="https://archives.openeuler.openatom.cn/openeuler/${set_version}/virtual_machine_img/aarch64/${QCOW2}"
    process_qcow2 "$QCOW2" "${QCOW2_URL}"
  done
fi

rm -f index.html qcow2.raw rootfs.img *.qcow2 *.xz
if wget --no-check-certificate "https://archives.openeuler.openatom.cn/openeuler/${set_version}/virtual_machine_img/x86_64/"; then
  ls -alh index.html
  for QCOW2 in $(grep -oE 'href="([^"]+\.qcow2.xz)"' index.html | cut -d'"' -f2); do
    rm -f qcow2.raw rootfs.img
    export QCOW2_URL="https://archives.openeuler.openatom.cn/openeuler/${set_version}/virtual_machine_img/x86_64/${QCOW2}"
    process_qcow2 "$QCOW2" "${QCOW2_URL}"
  done
fi
